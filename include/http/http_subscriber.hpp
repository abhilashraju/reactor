...
public:
  struct RetryPolicy {
    ...
    auto getRetryDelay() const { return std::chrono::seconds(retryDelay); }
  };
...
private:
  struct RetryRequest : std::enable_shared_from_this<RetryRequest> {
    ...
    ~RetryRequest() { CLIENT_LOG_INFO("RetryRequest destroyed"); }
    void setRequest(Request &&r) { req = std::move(r); }
    void waitAndRetry() {
...
public:
  ...
  ~HttpSubscriber() { CLIENT_LOG_INFO("HttpSubscriber destroyed"); }
...
private:
  ...
  void retryIfNeeded(Request &&req) {
    ...
    auto retryRequest =
        std::make_shared<RetryRequest>(std::move(req), retryPolicy, ioContext);
