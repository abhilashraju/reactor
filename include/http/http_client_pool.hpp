...
concept SessionInitialiser =
    requires(Handler h, std::shared_ptr<HttpSession<Stream>> session) {
      { h(session) } -> std::same_as<void>;
    };
template <typename Session> class HttpClientPool {
public:
  explicit HttpClientPool(net::any_io_executor executor, std::size_t pool_size)
      : context(executor), poolSize(pool_size) {}
  ~HttpClientPool() {
    while (pool.size() > 0) {
  ...
  std::shared_ptr<Session> acquire(auto intializer, Args &&...args) {
    ...
    auto it =
        std::ranges::find_if(pool, [](const auto &s) { return !s->inUse(); });
    if (it != std::end(pool)) {
      return *it;
    }
    if (pool.size() < poolSize) {
      pool.push_back(Session::create(context, std::forward<Args>(args)...));
