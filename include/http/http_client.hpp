...
protected:
  ...
  ~SyncStream() { CLIENT_LOG_DEBUG("SyncStream destroyed"); }
  auto &lowestLayer() { return beast::get_lowest_layer(mStream); }
...
public:
  void setErrorHandler(ErrorHandler handler) { fail = std::move(handler); }
...
protected:
  ...
  ~ASyncStream() { CLIENT_LOG_DEBUG("ASyncStream destroyed"); }
  auto &lowestLayer() { return beast::get_lowest_layer(mStream); }
  Stream &stream() { return mStream; }
  void on_resolve(std::function<void(beast::error_code)> connectionHandler,
  ...
  void
  on_write(std::function<void(beast::error_code, std::size_t)> onWriteHandler,
  ...
  void
  on_read(std::function<void(beast::error_code, std::size_t)> onReadHandler,
          beast::error_code ec, std::size_t bytes_transferred) {
    if (ec)
      return fail(ec, "read");
    onReadHandler(ec, bytes_transferred);
...
public:
  ...
  void setOption(Target t) { req_.target(t.target.data()); }
  void setOption(Port p) { port = p; }
  void setOption(Host h) { host = h; }
  void setOption(Verb v) { req_.method(v); }
  void setOption(Version v) { req_.version(v); }
  void setOption(KeepAlive k) { keepAlive = k; }
  void setOption(ContentType t) {
    req_.set(http::field::content_type, t.type.data());
  }
  template <typename... Options> void setOptions(Options... opts) {
    (setOption(opts), ..., 1);
  }
  void close() { stream->shutDown(); }
  ...
  bool inUse() const { return std::holds_alternative<InUse>(connectionState); }
  ...
  template <typename NewReqBody> auto cloneWithBodyType() {
    auto copystream = stream->makeCopy();
